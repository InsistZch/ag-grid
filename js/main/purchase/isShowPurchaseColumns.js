...
const isShowPurchaseColumns = (gridOptions) => {
    ...
    if (gridOptions.api) {
        ...
        // 显示隐藏非日采购和非今日采购和不需要显示的数据逻辑
        const isOrNOShow = () => {

            if (!noDailyProcurement.checked) { // 如果非日采购是非点击的状态
                gridOptions.api.forEachNode(v => { // 不是今日采购的不显示
                    if (v.key == null && v.data.purchase_freq != 'day') { // 所有
                        gridOptions.api.applyTransaction({ remove: [v.data] });
                    }
                })

            } else {
                gridOptions.rowData.forEach((v) => { // 点击状态
                    if (gridOptions.api.getRowNode(v.id) == undefined) { // 把所有隐藏的找到 gridOptions.api.getRowNode(v.id) == undefined
            ...
            gridOptions.api.forEachNode(v => {
                // id 为 noDaliyxxx的话就是 不是当前的日计划日期  || 需求日期小于今天 => 隐藏
